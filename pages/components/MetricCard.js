...
export default function MetricsCard(props) {
  return (
    <div className="rounded-2xl  bg-gray-50 p-6 border   border-gray-200 space-y-2 ">
      <span >
      ...
      <span className=" font-semibold text-sm ">{props.change.toFixed(1)}%</span></div>
      </span>
      <h1 className="text-left text-4xl font-bold">{numeral(props.count).format('0,0')}</h1>
    </div>
  );
}
