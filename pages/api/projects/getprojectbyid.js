    ...
    if (projectId.length === 24) {
      try {
        ...
        if (findResult) {
          ...
          const owner = await findResult.members.find(item => item.access === 'owner')
          if (privacy === 'public') {
            res.status(200).send(findResult);
          }else if (privacy === 'team') {
            
            if (findResult.members.find(item => item.email === session.user.email)) {
              console.log('is a member of project');
              res.status(200).send(findResult);
            } else {
              console.log('not a member of team')
              res.status(401).send('only members of this projects team may view this project')
            }
          } else {
            if (owner.email === session.user.email) {
              ...
              res.status(401).send('only the owner of this project may view it')
            }
          }
        }else {
